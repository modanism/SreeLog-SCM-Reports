...
-- Pre-aggregate the cost and revenue totals to ensure they are unique per BookingProcessId
;WITH CostRevAgg AS (
    SELECT
        BookingProcessId,
        SUM(CASE WHEN CostRev = 'Cost' THEN EstBaseAmount ELSE 0 END) AS TotalCost,
        SUM(CASE WHEN CostRev = 'Rev' THEN EstBaseAmount ELSE 0 END) AS TotalRevenue
    FROM #JobsheetCostRev
    GROUP BY BookingProcessId
),

-- Aggregate details at the carrier level to prepare for final consolidation
CarrierAgg AS (
    SELECT
        BookingProcessId,
        SUM([20]) AS Total20,
        SUM([40]) AS Total40,
        SUM([Other]) AS TotalOther
    FROM #CarrierDetails
    GROUP BY BookingProcessId
),

-- Join aggregated data and perform final calculations
-- Adjust the query to handle large numbers by converting integers to bigint or decimal
...
WITH ConsolidatedCarrierDetails AS (
  SELECT
    ...
    SUM(CONVERT(decimal(18,2), CASE WHEN cd.CostCurrency = 'USD' THEN 
          CASE
    ...
    SUM(CONVERT(decimal(18,2), CASE WHEN cd.RevCurrency = 'USD' THEN 
          CASE
    ...
    SUM(tc.RevenueTotal) AS 'Sell Total',
    SUM(tc.ProfitTotal) AS 'Total Profit'
